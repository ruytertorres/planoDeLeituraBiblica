 ...
 */
export function planoEstaPronto(plano) {
 ...
 */
export function getEstatisticasPlano(plano) {
  ...
  plano.dias.forEach(dia => {
    if (Array.isArray(dia.trechos)) {
      ...
      dia.trechos.forEach(trecho => {
        if (trecho.livro) livrosUnicos.add(trecho.livro);
        if (trecho.testamento) testamentos.add(trecho.testamento);
      });
    }
  });
