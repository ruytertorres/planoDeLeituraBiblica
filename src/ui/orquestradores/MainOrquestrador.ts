...
export class MainOrquestrador extends BaseOrquestrador {
  ...
  aplicarReajuste(lacuna?: { buracos?: number[]; tipo?: string }): {
    ...
    if (lacuna?.buracos && lacuna.buracos.length > 0) {
      // Caso de buracos esparsos: bloquear TODOS os dias não lidos
      // (incluindo o dia de retomada - ele só ficará disponível na data correta)
      diasBloqueados = lacuna.buracos;
    } else {
      // Caso de lacuna consecutiva: bloquear TODOS os dias do plano entre
      // o último dia lido e o dia atual do ano (não retomados)
      // Ex: último lido 3, hoje é 45 (14/02)
      // Bloquear dias 4-44 do plano (todos os dias que ficaram para trás)
