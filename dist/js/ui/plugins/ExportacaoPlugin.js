...
exports.ExportacaoPlugin = void 0;
    ...
    async exportarProgresso() {
        try {
            ...
            const dados = {
                ...
                dataExportacao: (0, timestampUtil_js_1.getTimestampAtual)(),
            };
  ...
  <footer style="text-align: center; color: #999; font-size: 12px;">
    ...
    async baixarDocx(conteudo, plano) {
        try {
            ...
            const { estatisticas, ultimoLido, proximo, notas, dataExportacao } = this.extrairDadosDoConteudo(conteudo);
            const dataFormatada = this.formatarData(dataExportacao);
            // Preparar seções do documento
            const secoes = [];
            // 1. Cabeçalho
            secoes.push(new window.docx.Paragraph({
            ...
            secoes.push(new window.docx.Table({
                rows: [
                    // Cabeçalho
                    ...
                    new window.docx.TableRow({
                        cells: [
                            ...
                            new window.docx.TableCell({
                                children: [
                                    new window.docx.Paragraph(`${estatisticas.diasLidos}/${estatisticas.totalDias}`),
                                ],
                            }),
                        ],
                    }),
            ...
            secoes.push(new window.docx.Paragraph({ text: "", spacing: { after: 300 } }));
            // 3. Última Leitura
            secoes.push(new window.docx.Paragraph({
                text: "📝 Última Leitura",
                heading: window.docx.HeadingLevel.HEADING_2,
            ...
            // Salvar arquivo
            const nomeArquivo = (0, timestampUtil_js_1.gerarNomeArquivoTimestamp)(`progresso_${plano.id}`, "docx");
    ...
    baixarDocxFallback(conteudo, plano) {
        ...
        const link = document.createElement("a");
        link.href = url;
        link.download = (0, timestampUtil_js_1.gerarNomeArquivoTimestamp)(`progresso_${plano.id}`, "html");
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
        URL.revokeObjectURL(url);
...
exports.ExportacaoPlugin = ExportacaoPlugin;
