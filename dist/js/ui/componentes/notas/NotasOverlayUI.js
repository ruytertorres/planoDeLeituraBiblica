...
exports.initNotasOverlayUI = initNotasOverlayUI;
const notas_toolbar_js_1 = require("../../../core/services/notas/notas_toolbar.js");
const NotasEnterHandler_js_1 = require("./NotasEnterHandler.js");
/**
 * Inicializa UI do overlay de notas
 * @param {NotasOverlayOrquestrador} orquestrador - Orquestrador de notas
 * @public
 */
function initNotasOverlayUI(orquestrador) {
    /* -----------------------------------------------------------------------
       Cache de elementos
    ----------------------------------------------------------------------- */
    ...
    if (toolbar) {
        (0, notas_toolbar_js_1.initNotasToolbar)(editor, toolbar);
    }
    /* -----------------------------------------------------------------------
       Handler de ENTER (função separada)
    ----------------------------------------------------------------------- */
    (0, NotasEnterHandler_js_1.setupEnterHandler)(editor);
    /* -----------------------------------------------------------------------
    ...
    editor.addEventListener("keydown", (e) => {
        ...
        const _getButton = (action) => {
            if (!_buttons[action]) {
                _buttons[action] = document.querySelector(`[data-nota-action="${action}"]`);
            }
    ...
    editor.addEventListener("keydown", (e) => {
        ...
        const _getHighlightButton = (color) => {
            if (!_highlightButtons[color]) {
                _highlightButtons[color] = document.querySelector(`[data-nota-action="highlight"][data-value="${color}"]`);
            }
