...
exports.validarPlanoStrict = validarPlanoStrict;
exports.planoValido = planoValido;
/* ============================================================================
...
function validarPlano(plano) {
    ...
    if (!Number.isInteger(planoObj.totalDias) || planoObj.totalDias <= 0) {
    ...
    ------------------------------------------------------------------------ */
    const dias = planoObj.dias;
    const totalDias = planoObj.totalDias;
    if (dias.length !== totalDias) {
        erros.push(`Plano inconsistente: totalDias (${totalDias}) difere da quantidade real de dias (${dias.length}).`);
    }
    /* ------------------------------------------------------------------------
...
function validarDia(dia, index) {
    ...
    if (diaObj.numero !== numeroEsperado) {
        erros.push(`dias[${index}]: número (${diaObj.numero}) fora de sequência. Esperado ${numeroEsperado}.`);
    }
 ...
 */
function validarEstruturaDia(dia) {
    if (!dia || typeof dia !== "object") {
        return false;
    }
    const diaObj = dia;
    return (typeof diaObj.numero === "number" &&
 ...
 */
function validarPlanoStrict(plano) {
